...
class Attacker:
    ...
    def attack_unconstrained(self, text_prompt, img, batch_size = 8, num_iter=2000, alpha=1/255):
        ...
        for t in tqdm(range(num_iter + 1)):
            ...
            if t % 100 == 0:
                ...
                prompt.update_context_embs()
                with torch.no_grad():
                    response, _ = my_generator.generate(x_adv.repeat(batch_size, 1, 1, 1),text_prompts)
                print('>>>', response)

                adv_img_prompt = denormalize(x_adv).detach().cpu()
                adv_img_prompt = adv_img_prompt.squeeze(0)
                save_image(adv_img_prompt, '%s/vap_%d.bmp' % (self.args.save_dir, t))
    ...
    def attack_constrained(self, text_prompt, img, batch_size = 8, num_iter=2000, alpha=1/255, epsilon = 128/255 ):
        ...
        for t in tqdm(range(num_iter + 1)):
            ...
            if t % 100 == 0:
                ...
                prompt.update_context_embs()
                with torch.no_grad():
                    response, _ = my_generator.generate_prompt(prompt)
                    # response = my_generator.generate(x_adv.repeat(batch_size, 1, 1, 1),text_prompts)
                print('>>>', response)
                # print("_", _)
    ...
    def attack_loss(self, prompts, targets):
        ...
        to_regress_embs = self.model.embed_tokens(to_regress_tokens.input_ids)
        ...
        bos_embs = self.model.embed_tokens(bos)
        ...
        pad_embs = self.model.embed_tokens(pad)
